...
class FightingStyles(commands.Cog):
    ...
    async def update_player_data(self, player_data: list[PlayerData]):
        ...
        roles = {
            ...
            for name, role_id in [
                ["Swordsman", 1015135602074718259],
                ["Sniper", 1015136299545526343],
                ["Doctor", 1015136303039402045],
                ["Brawler", 1015136309456666735],
                ["BlackLeg", 1015136312359145482],
                ["ArtofWeather", 1015136306113818714],
            ]
        }
        for player in player_data:
            ...
            if player.fighting_style:
                role = roles.get(player.fighting_style.name)
...
async def setup(bot):
    await bot.add_cog(FightingStyles(bot))
