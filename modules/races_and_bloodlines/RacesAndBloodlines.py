...
from discord import ButtonStyle, Embed, File, Interaction, Member, app_commands, ui
from discord.ext import commands
from discord.utils import get
from mcrcon import MCRcon
from utils.checks import is_bot_admin_interaction
from utils.database import get_user, models
from utils.objects import Bloodline, PlayerData, Race, Rarities


class BloodlineRequests(ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @ui.button(label="Linked", style=ButtonStyle.green, custom_id="persistent_view:green")
    async def green(self, interaction: Interaction, button: ui.Button):
        player = await models.Players.find_one(models.Players.discord_id == interaction.user.id)
        if not player:
            return await interaction.response.send_message(
                "You have not linked your discord with your minecraft account", ephemeral=True
            )
        role = interaction.guild.get_role(1014276690438000710)
        await interaction.user.add_roles(role)
        await interaction.response.send_message(f"**{player.name}** requests their bloodline.")


class RacesAndBloodlines(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.bot.add_view(BloodlineRequests())
        self.load_data()
        self.players = []
    ...
    async def enforce_race_blood(self, member):
        ...
        for race_blood in self.races + self.bloodlines:
            if race_blood.name in [user_data.race, user_data.bloodline]:
    ...
    @app_commands.command(name="roll_blood", description="Roll your bloodline.")
    async def roll_race_and_bloodlines(self, interaction):
        user_data = await get_user(interaction.user.id)
        # Races
        if user_data.race is None:
    ...
    async def reset_race_blood(self, interaction, user: Member, race: bool, blood: bool):
        ...
        if race:
            user_data.race = None
        if blood:
            user_data.bloodline = None
        await user_data.save()
        await self.check_race_and_blood(user)
        await interaction.response.send_message(f"Successfully reset {user.mention}.")
    ...
    async def give_bloodline_charm(self, interaction, player: str):
        ...
        data = await get_user(player.discord_id)
        if data.bloodline is None:
            return await interaction.response.send_message("That player hasn't rolled their bloodline.")
        blood = get(self.bloodlines, name=data.bloodline)
        charmIDGen = Random(blood.name)
        charmID = "".join(charmIDGen.choices(digits, k=4))
        with MCRcon(
            host=self.bot.config.server_ip, password=self.bot.config.rcon_password, port=self.bot.config.rcon_port
        ) as rcon:
            rcon.command(f"/curios clear {player.name} charm")
            rcon.command(f"/clear {player.name} apotheosis:potion_charm")
            charm = r'apotheosis:potion_charm{charmID:"$4",Unbreakable:1b,Enchantments:[{lvl:1,id:binding_curse},{lvl:1,id:"tombstone:soulbound"}],display:{Lore:["{\"text\": \"\"}","{\"text\": \"§fBloodline\"}","{\"text\": \"$1$2\"}"],Name:"{\"text\":\"$3\"}"}}'
            charm = (
                charm.replace("$1", blood.mc_color)
                .replace("$2", blood.tier_name)
                .replace("$3", blood.name)
                .replace("$4", charmID)
            )
    ...
    async def check_race_and_blood(self, member):
        ...
        race = None  # user_data.race
        blood = user_data.bloodline
